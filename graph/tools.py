...
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
# https://github.com/hulianyuyy/STGAT/blob/main/graph/tools.py


def get_expanded_graph(num_nodes, inward, type=1):
    # 25: upper_left,  26: upper_right,  27: up,  28: middle, 29: lower_left, 30: lower_right
    A_expand = np.zeros((num_nodes + 6, num_nodes + 6), dtype=np.float32)
    if type == 1:
        '''
        ...
        inward = inward + [(25, 4), (25, 5), (25, 6), (25, 7),
                           (25, 21), (25, 22), (25, 26), (25, 27)]
        inward = inward + [(26, 8), (26, 9), (26, 10),
                           (26, 11), (26, 23), (26, 24), (26, 27)]
        inward = inward + [(27, 2), (27, 3)]
        inward = inward + [(28, 0), (28, 1), (28, 20),
...
def get_strenghtened_graph(num_nodes, inward):
    ...
    A_strenghtened = A_strenghtened + \
        np.eye(len(A_strenghtened), dtype=np.float32)
...
def seperated_adjacency(A, k_list, with_self=True):
    ...
    for i in range(len(k_list)-1):
        k1 = k_list[i]
        k2 = k_list[i+1]
        Ak.append(np.minimum(np.linalg.matrix_power(A, k2), 1) -
                  np.minimum(np.linalg.matrix_power(A, k1), 1) + I)
