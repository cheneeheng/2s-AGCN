...
# Code refractored and ONLY the following points are included:
# - different pooling options
# - gcn fpn modes
# - different gcn fpn merging strategy
...
# - Embedding
# -- DataNorm
# - FeatureExtractor : pos & vel embeddings
# - SemanticEmbedding : temporal, spatial and class semantics
# -- OneHotTensor
# - GCNSpatialBlock : residual, weighted G, non shared G, ffn after gcn
# -- GCNSpatialG : shared g2, weighted G with another tensor
# -- GCNSpatialUnit : v format, no w2
# -- GCNSpatialFFN
# - TemporalBranch
# -- MLPTemporal : residual
# -- MHATemporal


# Continue from on sgn_v11
# NO PARTS
...
# Embedding modes ---
# 1 original sgn, 2 layer MLP, with act
# 2 original sgn, 2 layer MLP, with act, with res
# 3 original sgn, 3 layer MLP, with act
# 4 original sgn, 4 layer MLP, with act
# 101 bert style
EMB_MODES = [0, 1, 2, 3, 4, 101]
...
# 0: no merge
# 1: averaged and sent to 1 fc only.
# 2: no merge and used by multiple fc and averaged.
GCN_FPN_MERGE_MODES = [1, 2]
...
class SGN(PyTorchModule):
    ...
    def __init__(self,
                 ...
                 # INFOGCN
                 infogcn_noise_ratio: Optional[float] = None,
                 infogcn_gain: Optional[Union[int, float]] = None,
        ...
        assert self.input_position in EMB_MODES
        assert self.input_velocity in EMB_MODES
        assert self.semantic_joint in EMB_MODES
        assert self.semantic_frame in EMB_MODES
        assert self.semantic_class in EMB_MODES
        assert self.xem_projection in EMB_MODES
        ...
        elif self.semantic_joint_fusion == 0:
            self.gcn_in_ch = self.c1 * 2
        elif self.semantic_joint_fusion == 1:
            self.gcn_in_ch = self.c1
        else:
            raise ValueError("Unknown gcn_in_ch ...")
        ...
        if self.semantic_frame_location == 0:
            if self.gcn_fpn == 2:
                ...
                out_channels = sgcn_dims[-1] * len(sgcn_dims)
            elif self.gcn_fpn == 5:
                out_channels = sgcn_dims[-1]//4 * len(sgcn_dims)
            elif self.gcn_fpn == 6:
                out_channels = 64
            elif self.gcn_fpn == 8:
        ...
        # INFO GCN LOSS --------------------------------------------------------
        # https://github.com/stnoah1/infogcn/blob/master/model/infogcn.py
        if infogcn_noise_ratio is not None and infogcn_gain is not None:
            self.infogcn = True
            self.noise_ratio = infogcn_noise_ratio
            self.infogcn_gain = infogcn_gain
            self.z_prior = torch.empty(num_class, fc_in_ch)
            self.fc_mu = nn.Linear(fc_in_ch, fc_in_ch)
            self.fc_logvar = nn.Linear(fc_in_ch, fc_in_ch)
        else:
            self.infogcn = False
    ...
    def forward(self, x: Tensor) -> Tuple[Tensor, list]:
        ...
        if self.semantic_frame > 0 and self.semantic_frame_location == 1:
            x = fuse_features(x, tem_emb, self.semantic_frame_fusion)
        ...
        if self.semantic_frame > 0 and self.semantic_frame_location == 0:
            x_list = [fuse_features(i, tem_emb, self.semantic_frame_fusion)
        ...
        # temporal MLP
        _x_list = []
        for i, t_kernels in enumerate(self.multi_t):  # per fpn layer
            for j, t_kernel in enumerate(t_kernels):  # per t kernel
        ...
        if self.gcn_fpn_output_merge == 1:
    ...
    def init_weight(self):
        ...
        for p_name, param in self.sgcn.named_parameters():
            if f'w1.block.conv.conv.weight' in p_name:
                init_zeros(param)
        if self.infogcn:
            nn.init.orthogonal_(self.z_prior, gain=self.infogcn_gain)

    def latent_sample(self, mu: Tensor, logvar: Tensor) -> Tensor:
        if self.training:
...
class FeatureExtractor(PyTorchModule):
    def __init__(self,
                 in_pos: int = 0,
                 in_vel: int = 0,
                 in_pos_emb_kwargs: Optional[dict] = None,
                 in_vel_emb_kwargs: Optional[dict] = None,
                 ):
        super(FeatureExtractor, self).__init__()
        self.in_pos = in_pos
        self.in_vel = in_vel
        if self.in_pos > 0:
            assert in_pos_emb_kwargs is not None
            self.pos_embed = Embedding(**in_pos_emb_kwargs)
        if self.in_vel > 0:
            assert in_vel_emb_kwargs is not None
...
class SemanticEmbedding(PyTorchModule):
    def __init__(self,
        ...
        # Joint Embedding
        if self.sem_spa > 0:
            assert sem_spa_emb_kwargs is not None
        ...
        # Frame Embedding
        if self.sem_tem > 0:
            assert sem_tem_emb_kwargs is not None
        ...
        # Class Embedding
        if self.sem_cls > 0:
            assert sem_cls_emb_kwargs is not None
